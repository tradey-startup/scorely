...
Il sistema consente di gestire partite sportive con aggiornamento del punteggio in tempo reale utilizzando:
- **Braccialetti / telecomandi ESP32** come input fisico
- **Qualsiasi dispositivo con browser** (iPad, iPhone, tablet, PC) come tabellone
- **Tabellone ESP32 con display** come dispositivo opzionale
- **Infrastruttura cloud** come unica fonte di verità
...
La sessione:
- esiste nel cloud
- è indipendente dai dispositivi
- può essere ripresa da qualsiasi client autorizzato
...
|------------|------|------|
| Braccialetto ESP32 | input | invia eventi di punteggio |
| Web App (iPad/iPhone/Browser) | display / controller | tabellone principale |
| ESP32 Tabellone | display | opzionale |
| Browser Admin | admin | gestione location e storico |
...
|----------|-----------|------|
| Braccialetti ESP32 | Arduino / ESP‑IDF | Pulsanti + / − |
| Tabellone ESP32 | Arduino / ESP‑IDF | OLED / TFT |
| Broker MQTT | HiveMQ Cloud / Mosquitto | TLS |
| Web App | React + Tailwind CSS | MQTT over WebSocket |
| Cloud DB | Firebase Firestore / Supabase | Storico partite |
| QR Code | Libreria ESP32 / JS | Accesso rapido sessione |
...
Lo snapshot viene pubblicato:
- all'avvio sessione
- a ogni modifica punteggio
- su richiesta di un nuovo client
...
Il braccialetto:
- salva il topic in EEPROM / LittleFS
- invia solo eventi di gioco
...
|----|--------|-------|
| 1 | Setup ESP32 | Input funzionante |
| 2 | MQTT Cloud | Comunicazione stabile |
| 3 | Web App | Tabellone universale |
| 4 | Pairing | Associazione rapida |
| 5 | Snapshot state | Robustezza |
| 6 | Testing reale | Produzione |

---

## ✅ Conclusione
Questo sistema:
- funziona con o senza hardware dedicato
- è robusto in ambienti reali
- è pronto per essere prodotto e scalato
...
- non riscrivi codice
- non crei dipendenze inutili
- costruisci un prodotto solido
